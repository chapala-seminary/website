/* Exam engine for CTS1Peter (shared by 1 unit page).
   Extracted from those pages' inline <script>; per-unit config and
   question banks live in data/1peter/unitN.js. */
// =====================================================================
// CTS 1 Peter Intensive — Unit 12 (Shepherds and the Roaring Lion, 1 Peter 5:1-14)
...
function updateProgressGrid() {
    ...
    for (let i = 1; i <= totalUnits; i++) {
        ...
        grid.innerHTML += `<a href="CTS1PeterUnit${i}.html" class="${cls}" title="${unitTitlesEn[i-1]}">${i}</a>`;
    }
...
function checkLockouts() {
    ...
    if (unitPassed) {
        ...
        result.innerHTML = isEs
            ? `<span style='color:#1f6b3b'>✓ Unidad ya aprobada. Haga clic en ${UNIT < TOTAL() ? 'Unidad ' + (UNIT + 1) : 'el Certificado'} arriba.</span>`
            : `<span style='color:green'>✓ Unit already passed! Click ${UNIT < TOTAL() ? 'Unit ' + (UNIT + 1) : 'the Certificate'} above.</span>`;
